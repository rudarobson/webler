...
function DefaultImporter(importing: string, relativeTo?: string, alias?: string): Components.ImportResult[] {
  var imports: Components.ImportResult[] = [];
  var componentExt = '.component';
    ...
    if (this.type == markupType.comment) {
      ...
      if (commentMatch) {
        ...
        while (match = imports.exec(cnt)) {
          ...
          var dir;
          if (match[0][0] == '#') {
            results = DefaultImporter(cmpPath, currentSrcPath, alias);
          } else {
            results = DefaultImporter(cmpPath, null, alias);
          }
...
function _parse($, currentSrcPath, markupType, $root, options, templates) {
...
export = {
  start: function(input: any, options: Components.ComponentsOptions) {
    var opt = {};
    ...
    if (typeof (input) == typeof (''))
      input = $.parse(fs.readFileSync(input));


    var markupType = $.markupTypes;


    _parse($, options.srcFile, markupType, $(input), opt, {});
