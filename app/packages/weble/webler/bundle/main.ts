...
var weblerScript: WeblerScript = wRequire('weblerscript');
var dom = wRequire('dom');

function generateRandomPathInDir(dir: string): string {
  var p;
...
interface ExpandedFileType {
  src: string;
  readFrom: string;
  type: string;
}

function expandFiles(src: string, type: string, wp: WPManager, defaultFileType): ExpandedFileType[] {
  var srcs: ExpandedFileType[] = [];
...
function justCopyFiles(files: ExpandedFileType[], wp: WPManager, destCode, ext) {
  var gen = [];
