  ...
  while (i < query.length) {
    ...
    switch (query[i]) {
      ...
      case matchTypes.id:
        ...
        value = getIdentifierName(query, i + 1);
      ...
      case matchTypes.attribute:
        ...
        value = getAttributeMatch(query, i);
      ...
      case matchTypes.pseudo:
        ...
        value = getPseudo(query, i, pseudoList);
        i += value.full.length;
      ...
      default:
        if (!selector) {
          ...
          value = getIdentifierName(query, i);
...
function parse(tokenStreams) {
  ...
  for (var i in tokenStreams) {
    ...
    var j = tokenStream.length - 2;
    var childMatch = tokenStream[j + 1];

    while (j >= 0) {
      var tk = tokenStream[j];
      switch (tk.type) {
        case tkTypes.combinator:
          contexts.push({
            type: tk.value,
            match: tokenStream[j - 1]
          });
          j--;
          break;
...
var executeTypes = {
  anyParent: matchTypes.seaparator,
...
export = {
  parse: function(query, opt) {
    if (query._type == 'parsedcssselector')
      return query;
    var ret: any = parse(Tokenize(query, opt));
