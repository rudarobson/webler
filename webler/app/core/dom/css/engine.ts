...
function elementAgainstSelector(elt, match) {
  ...
  if (match.tagName && !(match.tagName.toLowerCase() == elt.tagName.toLowerCase()))
    return false;
...
export = {
  is: function(query, elt) {
    var parsed = selectorParser.parse(query);

    var match = false;

    if (elt.type == mtypes.document)
      return false;
    ...
    for (var j in allPossibleRules) {
      ...
      while (curElt.parent.type != mtypes.document && i < rule.combinators.length) {
