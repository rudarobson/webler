...
function defineReference(type, fCtx, destRef, wp) {
  var absDestRef = wp.vp.resolveDest(destRef);
  var relPath = path.relative(wp.vp.vDest(), wp.vp.resolveDest(destRef));

  if (path.isAbsolute(relPath)) { //cannot be replace by a relative path
    return '/' + wp.vp.trim(destRef);
...
var bundleApi = {
  parse: function(content, opts, wp, globalOptions, fCtx) {
    var obj;
    var vSrc = opts.vSrc;
    ...
    for (var i in bundleRegex) {
      ...
      while ((match = regex.exec(content))) {
        ...
        var newRef = defineReference(i, fCtx, match[1], wp);
