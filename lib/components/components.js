...
function _registerTemplate(name, path, options) {
	if (templates[name] && templates[name].path != path) {
		...
		console.log(message);
		if (options.parseAllHyphenTags) //stop if must parse all tags
			throw message;
	}
...
function _loadTemplate(name, options) {
	if (templates[name]) {
		if (!templates[name].cnt) {
			var newPath = path.join(options.componentsPath, templates[name].path + options.componentExt);
...
function _parseConfiguraion(src, options) {
	...
	if (cntMatch) {
		...
		while (match = imports.exec(cnt)) {
			...
			if (isDirectory) {
				var files = glob.sync('**/*' + options.componentExt, {
					cwd: options.componentsPath
				});

				for (var i in files) {
					var pathWithoutExt = files[i].slice(0, -(options.componentExt.length)); //trim extension
...
function _parseTagWithContent(template, cnt, options) {
	...
	_attrEngine(templateAttrs, tagAttrs, options.attrAction, options.attrsToMerge, options.attrsToReplace);
