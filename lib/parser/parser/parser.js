  ...
  this.parse = function(html) {
    ...
    while (match = tagRegex.exec(html)) {
      if (match.index > lastTextIndex) {
        var text = createText(html.substring(lastTextIndex, match.index));
        context[0].append(text);
        lastTextIndex = tagRegex.lastIndex;
      }

      //match is always something like <tag-representation...>

      if (commentStartRegex.test(match[0])) { //it's comment
        //must find the end of comment because the regex will find first '>', and start nexte iteration over there

        var nextIndex = html.indexOf('-->', match.index);
        if (nextIndex > -1) {
          tagRegex.lastIndex = nextIndex + 3; //skip the -->
        }
        var tag = createTag(html.substring(match.index, tagRegex.lastIndex));
        context[0].append(tag);
      } else if (doctypeStartRegex.test(match[0]) || cdataStartRegex.test(match[0])) {
        ...
        if (/<\s*\//.test(match[0])) { //closing tag remove context
          ...
          while (i < context.length) {
            ...
            if (context[i].tagName.toLowerCase() == match[1].toLowerCase()) {
              if (context[i].children) { //if not the topLevelElts array
                context[i].children.splice(0, i + 1);
              }
              break;
            }
            i++;
...
var elts = new Parser().parse(fs.readFileSync('./test.html').toString());
