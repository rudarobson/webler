...
function Webler(files, options) {
  ...
  if (!allDestFolders[options.dest])
    allDestFolders[options.dest] = true;

  options.temp = wp.vp.resolveDest(options.temp);
  if (!allTempFolders[options.temp])
    allTempFolders[options.temp] = true;
...
function solveGlobs(globs, srcRoot, destRoot) {
  ...
  for (i in globs) {
    ...
    for (var i in virtualSrcs) {
      var virtualSrc = virtualSrcs[i];
      if (virtualSrc.indexOf('~') === 0) {
        virtualSrcs[i] = virtualSrc.substr(1);
      }
      ...
      virtualSrcs = vp.resolveSrc(virtualSrc);
    }
...
module.exports = {
  ...
  weble: function(options) {
    ...
    var files = solveGlobs(globs, opt.src, opt.dest);

    return new Webler(files, opt);
  },
